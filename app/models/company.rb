...
class Company < ApplicationRecord
 ...
 "ZTS"]
  ...
  def self.create_companies
    ...
    puts "#{api_key} HEEEERE"

    ENV['SP500'].split(',').each do |ticker|
    # ['IBM'].each do |ticker|
  ...
  def self.high_yield
    Company.where('dividend_yield > 0.04').order(:sector, :dividend_yield).pluck(:ticker, :name, :sector, :dividend_yield)
  end
end
